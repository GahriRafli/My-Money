...
export function usePushNotification(user, householdId) {
  ...
  async function subscribe() {
    ...
    console.log("[Push] subscribe checks:", checks);
    if (!checks.supported || !checks.user || !checks.householdId || !checks.vapidKey) {
      const missing = Object.entries(checks).filter(([,v]) => !v).map(([k]) => k).join(", ");
      alert(`[Push] Gagal: missing ${missing}`);
      return;
    }
    setLoading(true);
    try {
      ...
      setPermission(perm);
      if (perm !== "granted") {
        alert(`[Push] Permission tidak granted: ${perm}`);
        setLoading(false);
        return;
      }
