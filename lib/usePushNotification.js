...
export function usePushNotification(user, householdId) {
  ...
  async function subscribe() {
    ...
    try {
      ...
      console.log("[Push] upserting payload:", JSON.stringify(payload));
      ...
      if (error) {
        console.error("[Push] upsert error:", JSON.stringify(error));
      } else {
        localStorage.setItem(ENDPOINT_KEY, sub.endpoint);
        console.log("[Push] saved to DB ✓", JSON.stringify(upsertData));
      }
